...
def recover_watermark(image_array, model='haar', level=1):
    ...
    # print_image_from_array(watermark,)
    print(np.min(watermark_array))
    print(np.max(watermark_array))
    for i in range(watermark_array.shape[0]):
        for j in range(watermark_array.shape[1]):
            if watermark_array[i,j]<5 or watermark_array[i,j]>40:
                watermark_array[i,j]=0
            else:
                watermark_array[i,j]=255
...
# Step 3: Apply the filter to the image using convolution
    ...
    print_image_from_array(image_array,'orig.jpg')
